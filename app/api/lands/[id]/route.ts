...
export async function PUT(
  ...
  try {
    ...
    const { _id: resultDbId, ...responseLand } = result;

    return NextResponse.json({ land: responseLand });
  } catch (error) {
    console.error(`Error in PUT /api/lands/${id}:`, error);
...
export async function DELETE(
  ...
  try {
    ...
    console.error(`Error in DELETE /api/lands/${id}:`, error);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
